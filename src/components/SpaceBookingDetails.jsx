...
import {BOOKING_PREVIEW_URL, GET_TOKEN} from "../utils/constants.jsx";
...
const SpaceBookingDetails = ({
    ...
    const [priceForAllSpace, setPriceForAllSpace] = useState(0);
    ...
    useEffect(() => {
        fetchBookingPreview();
    }, [sDate, eDate, requiredSpace, parkingSpot])
    ...
    useEffect(() => {
        if (preview) {
            setPrice((preview?.basePricePerNightPaise / 100).toFixed(2));
            setPriceForAllSpace((preview?.subtotalPaise / 100).toFixed(2));
            setServicetax((preview?.feesPaise / 100).toFixed(2));
            setTotalPrice((preview?.totalPaise / 100).toFixed(2));
        }
    }, [preview]);
    ...
    const fetchBookingPreview = () => {
        fetch(BOOKING_PREVIEW_URL, {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                Authorization: "Bearer " + GET_TOKEN(), // JWT here
            },
            body: JSON.stringify({
                listingId: parkingSpot?.id,
                startDate: sDate,
                endDate: eDate,
                spacesQty: Number(requiredSpace),
            }),
        }).then((res) => {
            // console.log(res);
            if (!res.ok) {
    ...
    return (
        <div className="bg-white rounded-2xl border border-neutral-200 shadow-[0_10px_30px_rgba(2,6,23,0.08)] p-4">
            ...
            <div className="flex items-center justify-between font-semibold">
                <div>{parkingSpot?.capacitySpaces}</div>
                <div>{price}/day</div>
            </div>
            ...
            <div className="mt-4">
                ...
                <input
                    ...
                    value={requiredSpace}
                    min={1}
                    max={parkingSpot?.capacitySpaces}
            ...
            {!isCheckOutPage ? (
                ...
                >
                    <button
                        className="mt-3 h-11 w-full rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-semibold">
                        Proceed
                    </button>
                </Link>
            ) : (
            ...
            {isCheckOutPage ? (
                <>
                  ...
                  ${canRequest ? "bg-red-500 hover:bg-red-600 text-white" : "bg-neutral-200 text-neutral-500 cursor-not-allowed"}`}
                        onClick={() => handleRequestToBook(sDate, eDate,requiredSpace)}
                    >
                        Request to book
                    </button>
                </>
