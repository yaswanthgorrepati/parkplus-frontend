      ...
      <div className="p-4">
        <div className="flex  justify-between">
          <div className="flex items-center  flex-wrap gap-2 w-full">
            {item?.badgeTypes?.length > 0 &&
              item.badgeTypes.map((badge) => (
                <span
                  key={badge}
        ...
        <div className="flex flex-col mt-4 border-t border-neutral-200 pt-4">
          <div className="flex justify-between mb-2">
            <div className="text-neutral-500 text-sm">
              {item.capacitySpaces} parkings
            </div>
            <div className="text-green-600 font-medium">₹{(Number(item.basePricePerDayPaise)/100).toFixed(2)}</div>
          </div>
